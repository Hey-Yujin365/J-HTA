...
집합 연산자
    ...
    형식
        ...
        SELECT 컬럼, 컬럼, 컬럼
        FROM 테이블2
        [WHERE 조건식] ... -- 집합연산 개수의 제한은 없다.
    
    제약사항
        1. SELECT문의 컬럼수가 동일해야 한다.
        2. SELECT절의 동일한 위치에 존재하는 컬럼의 데이터 타입이 상호 호환 가능해야 한다.
           (데이터타입이 반드시 동일한 타입일 필요는 없다.)
    
    종류
        UNION
            여러 SQL문의 결과에 대한 합집합 결과를 만든다.
...
-- INTERSECT
-- 급여를 5000달러 이하를 받는 직원들 집합과 직종이 변경된 적이 있는 직원을 집합의 교집합 조회하기
SELECT EMPLOYEE_ID
FROM EMPLOYEES
...
-- EXISTS 활용해서 사용하기. IN으로 쓰면 JOB_HISTORY에 많은 데이터가 들어있을 경우 성능이 떨어진다.                    
...
EXISTS 연산자
    ...
    + 실제 프로젝트에서 특정 조건을 만족하는지 여부를 묻는 로직이 많이 사용되는데,
      COUNT(*)로 조건을 만족하는 행의 개수를 조회하는 것은 SQL 실행 성능에 나쁘다.
      따라서, EXISTS 연산자를 사용할 수 있는지 고려해야 한다.
    + EXISTS (서브쿼리)에서 SELECT절에는 업무족으로 의미없는 상수값(1, 'X')을 반환하도록 한다.
...
WHERE 0 < (SELECT COUNT(*)
           FROM JOB_HISTORY H
           WHERE H.EMPLOYEE_ID = E.EMPLOYEE_ID);
           -- 모든 EMPLOYEES 의 행마다 서브쿼리 속 COUNT를 다 세기 때문에 성능이 떨어진다
SELECT EMPLOYEE_ID, FIRST_NAME
FROM EMPLOYEES E
WHERE EXISTS (SELECT 'X'
              FROM JOB_HISTORY H
              WHERE H.EMPLOYEE_ID = E.EMPLOYEE_ID);
              -- EXISTS는 하나라도 발견하면 더이상 검색하지 않기 때문에 성능면에서 위의 코드보다 낫다
