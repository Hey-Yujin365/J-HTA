...
public class LottoService {
	 ...
	 * 4. Order 객체를 생성한다.
	 * 5. Order 객체에 회차번호, 주문번호, 로또번호 리스트를 저장한다.
	 * 6. Repository객체에 Order 객체를 전달해서 저장시킨다.
	...
	private Set<Integer> generateSet() {
		...
		while (true) {
			...
			// TreeSet객체에 위에서 획득한 숫자를 저장한다.(같은 숫자는 저장되지 않음)
			set.add(num);
			// TreeSet 객체에 6개가 저장되면 반복을 중단한다.
			if (set.size() == 6) {
				break;
			}
		}
	...
	public Order getOrder(int lottoNo, long orderNo) {
		...
		for (Order order : orders) {
			// 주문정보의 주문번호와 전달받은 주문번호가 일치하면 해당주문정보를 반환한다.
	 ...
	 * 5. Lotto 객체에 회차번호, 로또번호세트, 보너스번호를 저장한다.
	 * 6. Repository 객체에 Lotto객체를 전달해서 저장시킨다.
	 * 7. Lotto 객체를 반환한다.
	 * @return 로또 당첨번호 정보
	 */
	public Lotto lotto() {
	 ...
	 * 3. 해당 세트의 당첨정보를 저장할 Winning 객체를 생성한다.
	 * 4. Winning 객체의 당첨여부를 기본적으로 true로 설정한다.
	 * 5. check(로또번호, 구매한 로또번호세트)를 실행해서 맞는번호세트를 조회한다.
	 * 6. 맞은 번호 개수를 획득한다.
	 * 7. 맞은 번호 개수를 기준으로 등수를 결정한다.
	 * 8. 당첨정보를 저장하는 Winning 객체에 구매한 로또번호세트, 맞은 개수, 맞은 번호세트, 등수를 저장한다.
	 * 9. 1번에서 생성한 ArrayList 객체에 당첨정보를 저장한다.
	 * 2. 반복종료
	 * 10. 당첨정보를 반환한다.
	...
	private List<Winning> check(Lotto lotto, List<Set<Integer>> numbers) {
		...
		for (Set<Integer> set : numbers) {
			...
			// 당첨여부를 true로 설정하기, 나중에 맞은 개수가 3개 미만인 경우 false로 설정할 예정
			winning.setWin(true);
			// check(추첨번호, 구매한로또번호세트)를 실행해서 맞은 번호를 포함하는 Set객체를 획득하기
