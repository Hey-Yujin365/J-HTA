...
다중행 함수
    ...
    * 집합그룹 : 테이블 전체
    ...
    COUNT(*)
        조회된 모든 행의 개수를 반환한다.
    COUNT(컬럼명)
        조회된 행에서 지정된 컬럼의 값이 NULL이 아닌 행의 개수를 반환한다.
    SUM(컬럼명)
        조회된 행에서 지정된 컬럼 값의 합계를 반환한다.(NULL은 무시한다.)
    AVG(컬럼명)
        조회된 행에서 지정된 컬럼 값의 평균을 반환한다.(NULL은 무시한다.)
    MIN(컬럼명)
        조회된 행에서 지정된 컬럼의 최솟값을 반환한다.(NULL은 무시한다.)
    MAX(컬럼명)
        조회된 행에서 지정된 컬럼의 최댓값를 반환한다.(NULL은 무시한다.)
    
    다중행 함수 작성시 주의사항
        - 다중행 함수는 WEHRE절에 사용할 수 없다.
          SELECT *
          FROM EMPLOYEES
          WHERE COUNT(*) > 10   -- 오류다.
        - 다중행 함수와 일반 컬럼명을 SELECT절에 같이 적을 수 없다.
          ...
          GROUP BY DEPARTMENT_ID 를 추가해주면 가능하다.
        - 다중행 함수의 중첩은 한번만 허용된다.
          그룹 함수를 2번 하면, 결과는 한 행만 나온다.
          그렇기 때문에 중첩은 한번만 허용된다.
...
그룹화하기
    ...
    * GROUP BY절을 사용하면 테이블의 행을 그룹화하고, 그룹화된 각 그룹에 다중행 함수를 적용해서 실행시키고
      ...
      * 직종별 최고급여, 최저급여, 평균급여, 급여총합, 행의 개수 등을 조회할 수 있다.
      * 입사년도별 최고급여, 최저급여, 평균급여, 급여총합, 행의 개수를 조회할 수 있다.
      * 급여등급별 최고급여, 최저급여, 평균급여, 급여총합, 행의 개수를 조회할 수 있다.
*/
