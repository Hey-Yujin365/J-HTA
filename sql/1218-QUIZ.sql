...
-- Neena와 동일한 급여를 받는 모든 사원의 이름과 입사일 급여를 조회하기, 결과에 Kochhar은 포함시키지 않기
...
-- 다시풀기
SELECT C.MANAGER_ID, B.MGR_NAME, C.CNT
FROM (SELECT E.FIRST_NAME MGR_NAME
      FROM EMPLOYEES E
      WHERE Z.MANAGER_ID = E.EMPLOYEE_ID) B,
     (SELECT E.MANAGER_ID, COUNT(*) CNT
      FROM EMPLOYEES E
      WHERE MANAGER_ID IS NOT NULL
      GROUP BY MANAGER_ID) C,
      EMPLOYEES Z
WHERE Z.EMPLOYEE_ID = C.MANAGER_ID;