...
DECODE 함수
    ...
    * 지정된 컬럼의 값이 비교값1과 같으면 값1이 반환된다.
                       ...
                       기본값은 생략 가능하다.
    * DECODE함수는 컬럼의 값과 비교값간의 equal 비교만 가능하다. 
    * 값 자리에 SELECT문이 와도 된다.
...
CASE ~ WHEN 표현식
    ...
    * DECODE함수와 비교했을 때 더 다양한 조건을 적용할 수 있다.
    ...
    CASE 컬럼
        ...
        ELSE 값4
    END
    * 지정됨 컬럼의 값이 비교값들 중 하나와 일치하면 해당 THEN의 값이 최종값이 된다.
    * 모든 비교값과 일치하지 않으면 ELSE의 값4가 최종값이 된다.
    * DECODE 함수와 기능면에서 동일하다.(equal비교만 가능하다)
*/
...
-- EQUAL 비교일 때는 아래와 같은 방식으로 적어도 동일한 결과가 나온다.
SELECT REGION_ID,
        CASE REGION_ID
            WHEN 1 THEN '유럽'
            WHEN 2 THEN '아메리카'
