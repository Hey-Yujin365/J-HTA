...
public class Car {
	 ...
	 *			 결정할 수 있게 한다.
	 * + 메소드명: 메소드가 수행하는 기능울 표현하는 적잘한 이름으로 지정한다.
	 * 			 식별자 작성 규칙을 따른다.
	 * + 매개변수: 메소드를 호출하는 측이 메소드에게 인자로 전달한 값을 저장하는 변수다.
	 * 			 매개변수의 타입과 이름을 통해서 인자로 전달한 값과 값의 타입을 
	 * 			 유추할 수 있게 한다.
	 * + 수행문: 메소드의 고유 기능을 수행한다.
	 * + return 문 : 메소드의 최종 반환값을 메소드를 호출한 측에게 반환한다.
	 * 				 반환타입이 void 가 아닌 경우에는 반드시 return 문이 정의되어 있어야 한다.  
	 */
	...
	void changeCurrentSpeed(int speed) {
		// 값을 전달받아서 자동차의 현재 속도를 전달받은 값으로 변경하는 메소드
		// 반환타입: 없음
		// 매개변수: 변경할 속도값, int
