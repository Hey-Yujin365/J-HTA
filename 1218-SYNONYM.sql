...
스키마(SCHEMA)
    ...
    * 다른 사용자가 소유하고 있는 데이터베이스객체를 액세서할 때는 반드시 스키마를 명시해야 한다.
*/
-- HTA에게 객체권한이 부여된 HR의 EMPLOYEES 테이블 조회하기
SELECT *
FROM HR.EMPLOYEES; -- 여기서 HR이 스키마
...
시노님을 사용하는 이유
    시노님은 다른 유저의 객체를 참조할 때 많이 사용한다.
    객체의 긴 이름을 사용하기 쉬운 짧은 이름으로 시노님을 생성하면 SQL 코딩을 단순화할 수 있다.
    시노님을 사용하면 사용자가 참조하고 있는 객체에 대한 소유자, 이름을 감출 수 잇다.
...
/*
HR 계정으로 접속해서 EMPLOYEES 테이블에 대한 객체권한을 HTA에게 부여하기
...
-- 시노님을 사용하면 실제 데이터베이스 객체의 소유자, 실제 데이터베이스 객체 이름을 감출 수 있다.
CREATE SYNONYM EMP
FOR HR.EMPLOYEES;
