...
public class BankAccount {
	 ...
	 * 이전 비밀번호와 비밀번호가 일치하고,
	 * 새 비밀번호와 이전 비밀번호가 서로 다른 경우에만 비밀번호를 변경한다.
	...
	public void changePassword(int oldPwd, int newPwd) {
		if (oldPwd != password) {
			...
			return;		// 반환타입 void일 때 return을 적으면 즉시 종료된다.
		}
		
		if (oldPwd == newPwd) {
			System.out.println("### 비밀번호 변경 실패: 기본 비밀번호와 같은 번호로 변경할 수 없습니다.");
			return;
		}
	...
	public long terminate(int pwd) {
		...
		System.out.println("### 계좌가 해치처리되었습니다.");
