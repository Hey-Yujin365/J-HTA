...
TOP-N 분석과 분석함수
    ...
    형식
        ...
        * ORDER BY절로 데이터를 정렬한 후 ROWNUM으로 몇 개의 데이터만 추출해야 하는 경우,
          ORDER BY절을 먼저 처리하도록 인라인 뷰를 만들고, 그 뷰를 감싸는 바깥쪽 SQL에서 ROWNUM 처리를 하자.
*/

-- 직원들을 급여순으로 정렬했을 때 급여를 가장 많이받는 직원 5명(1등~5등)의 직원아이디, 이름, 급여, 직종아이디를 조회하기
...
-- 아래 코드는 TON-N 분석은 아니다.
-- 직원들을 급여순으로 정렬했을 때 급여를 가장 많이받는 직원 5명(6등 ~ 10등)의 직원아이디, 이름, 급여, 직종아이디를 조회하기
...
오라클의 분석함수
    ...
    형식
        SELECT 분석함수([컬럼]) OVER ([PARTITION B 컬럼] [ORDER BY 컬럼]) 
        FROM 테이블명           
*/
...
RANK 함수
    ...
    중복 순위 다음은 해당 데이터의 개수만큼 증가된 값을 반환한다.
*/
...
ROW_NUMBER 함수
    ...
    ROW_NUMBER 함수는 같은 값이어도 고유한 순번이 부여된다.
